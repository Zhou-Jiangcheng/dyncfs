...
def pre_process_qssp2020(
    ...
    green_info = {
        ...
        "event_depth_list": list(event_depth_list),
        "receiver_depth_list": list(receiver_depth_list),
        "spec_time_window": float(spec_time_window),
        "sampling_interval": float(sampling_interval),
        "max_frequency": float(max_frequency),
        "max_slowness": float(max_slowness),
        "anti_alias": float(anti_alias),
        "turning_point_filter": int(turning_point_filter),
        "turning_point_d1": float(turning_point_d1),
        "turning_point_d2": float(turning_point_d2),
        "free_surface_filter": free_surface_filter,
        "gravity_fc": float(gravity_fc),
        "gravity_harmonic": int(gravity_harmonic),
        "cal_sph": int(cal_sph),
        "cal_tor": int(cal_tor),
        "min_harmonic": int(min_harmonic),
        "max_harmonic": int(max_harmonic),
        "source_radius": float(source_radius),
        "source_duration": float(source_duration),
        "output_observables": list(output_observables),
        "time_window": float(time_window),
        "sampling_num": round(time_window / sampling_interval) + 1,
        "time_reduction": float(time_reduction),
        "grn_dist_range": list(dist_range),
        "grn_delta_dist": float(delta_dist),
        "path_nd": str(path_nd),
        "path_nd_without_Q": str(path_nd_without_Q),
        "earth_model_layer_num": int(earth_model_layer_num),
        "physical_dispersion": int(physical_dispersion),
    }
...
def create_grnlib_qssp2020_spec_parallel_multi_nodes(path_green, check_finished=False):
    try:
        ...
        raise ValueError("Please install mpi and mpi4py")
